...
const {exec,time} = useIntervalRequest(10,(data:any)=>{
 return add(data)
})
</script>
<template>
    ...
    <section class="py-3">
      <!-- <Form :validation-schema="schema" #default="{handleSubmit}" :validateOnMount="false" :key="collections.length"> 
        <Field name="content" class="border" v-model="model.content" :validate-on-input="true"> -->
          ...
          <el-button type="primary" :disabled="time>0?true:false" size="default" class="mt-3" @click="exec(model)">发表评论{{time>0?`(${time})`:''}}</el-button>
        <!-- </Field>
